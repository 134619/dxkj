                ...
                if (field_name.endswith(endswith_str) or field_name.endswith(endswith_time_str)) and len(
                    ...
                    if start_date_valid and end_date_valid:
                        if 'date' in field_name and 'time' in field_name:
                            where_prefix_part = '{} BETWEEN TIMESTAMP {} AND TIMESTAMP {}'.format(field_name,
                                                                                                  repr(field_value[0]),
                                                                                                  repr(field_value[1]))
                        else:
                            where_prefix_part = '{} BETWEEN TIMESTAMP {} AND  TIMESTAMP {}'.format(field_name,
...
# 分表查询功能
def get_sub_table(table_name, first_field: str | list = '', second_field_1: str | list = '',
                  second_field_2: str | list = '', year_period_tp: tuple[str, str] = tuple(), key=None):
    """
    入参：
    table_name 表名
    first_field 第一分表字段
    ...
    if table_name_ls:
        for table_name in table_name_ls:
            if key:
                ...
                data_list = db.get_table_columns(table_name)
                col_sql = ','.join(f'`{col["name"]}`' for col in data_list)
            union_table_ls = []
            table_sql = """
            SELECT {} FROM `{}`
