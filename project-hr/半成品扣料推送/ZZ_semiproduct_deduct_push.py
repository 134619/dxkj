...
from utils import (
    insert_db_records,
    query_db_records_by_cond,
    update_db_record_by_cond,
)
...
class SemiproductDeductPush:
    ...
    def _push_core(self, user_id, task_id, year, period, plant_code,
        ...
        if record_cnt:
            ...
            try:
                ...
                self._update_summary_result(unique_records, resp, year, period, plant_code)
                if stat == "2":
                ...
                self._update_summary_result(unique_records, fail_resp, year, period, plant_code)
        # 回查最终状态(供前端刷新表格)
    ...
    def _update_summary_result(self, records, resp, year="", period="", plant_code=""):
        """回写结果表 + 源表: 物料凭证号(MBLNR)/金额(DMBTR)/年度/状态/消息

        状态映射(新逻辑): SAP 成功(stat='2') -> 表里存 3; 失败(stat='D'等) -> 表里存 2。
        两张表共用同一映射(由 _map_posting_status 统一换算):
          t_co_summary_result: UPSERT(存在改/不存在建) associated_doc_sn/amount/status..., 按唯一键 summary_type+summary_unique_number+summary_line
        ...
        # 结果表 t_co_summary_result: 按唯一键 (summary_type, summary_unique_number, summary_line) 先查后写
        # (该库不支持 ON DUPLICATE KEY, 故手动: 存在 -> UPDATE; 不存在 -> INSERT。prodosn 不可作定位键)

        
        # P_WLPZ 与推送行序一一对应: 第 idx 行 MBLNR -> associated_doc_sn, DMBTR -> amount
        p_wlpz = resp.get("p_wlpz") or []
        update_cols = ("associated_doc_sn", "amount", "year", "status", "post_account_msg", "update_time")
        insert_cols = ("summary_type", "summary_unique_number", "summary_line", "year", "period", "plant_code",
                       "associated_doc_sn", "amount", "status", "post_account_msg", "reverse_account_msg", "update_time")
        for idx, record in enumerate(records):
            ...
            if not summary_type or not summary_unique_number:
                continue
            print("summary_unique_number",summary_unique_number)
            print("summary_line",summary_line)
            ...
            # 先查结果表是否已有该唯一键的行: 有则更新, 无则插入
            # if query_db_records_by_cond(self.db, self.RESULT_TABLE, key_cond, "id"):
            status = update_db_record_by_cond(self.db, self.RESULT_TABLE, key_cond, update_cols, row_data)
            print('status:1111111', status)
            # else:
            #     ins = dict(row_data)
            #     ins["summary_type"] = summary_type
            #     ins["summary_unique_number"] = summary_unique_number
            #     ins["summary_line"] = summary_line
            #     ins["period"] = period
            #     ins["plant_code"] = plant_code
            #     ins["reverse_account_msg"] = ""   # NOT NULL 约束, 新建行先置空串
            #     insert_db_records(self.db, self.RESULT_TABLE, insert_cols, [ins])
        self.db.dbCommit()
...
body = {'summary_unique_number': 'f0186d008156762431e8fb995a99a64e','movement_type':'262','summary_type': 11, 'plant_code': 'RAC1', 'period': '07', 'path': '', 'year': '2026'}
