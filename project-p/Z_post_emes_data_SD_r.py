...
def post_data():
    ...
    #     # 处理多个交货单/调拨单数据
    #     convert_and_process_file_payload(payload,user_id)
    # else:
    data = body.get("data")
    # print('data-----', data)
    # print('body-----', body)
    ret = sd_post_data_execute(data,user_id)
    if ret['type'] == 'E':
        res.set_body(json.dumps(ret))
        res.commit(True)
    else:
        res.set_body(json.dumps({"code": 200, "msg": "执行完成", "data": data}))
        res.commit(True)
...
def sd_post_data_execute(data, user_id, dict_head=None):
    ...
    if uf_bs_category == '29':
        ...
        # 构建数据
        # 调用过账接口
        ret = build_post_params(dict_head, user_id)
        if ret['type'] == 'E':
            return ret
        doc_head = ret['data']['doc_head']
        doc_items = ret['data']['doc_items']
        execute_post(doc_head, doc_items, guid, user_id)
...
def build_post_params(header, user_id):
    ...
    for item in header.get("item", []):
        ...
        plant_code = item.get("uf_company_code", "")

        uf_guid = item.get("uf_guid", "")
        if mat_code or batch_sn or plant_code is None:
            return {
                "type": "E",
                "message": f"第{line_id}行：物料代码、批次号、工厂代码不能为空",
                "guid": uf_guid,
                "data": None
            }
